...
run_neptune = neptune.init_run(
    project="ahn-group/spherenet-3dpe",
...
class run():
    ...
    def run(self, target, device, train_dataset, valid_dataset, test_dataset, model, loss_func, evaluation, seed, pe,k,sigma,num_layers, epochs=500, batch_size=32, vt_batch_size=32, lr=0.0005, lr_decay_factor=0.5, lr_decay_step_size=50, weight_decay=0, 
        ...
        valid_loader = DataLoader(valid_dataset, vt_batch_size, shuffle=False,generator=torch.Generator(device=device))
        test_loader = DataLoader(test_dataset, vt_batch_size, shuffle=False,generator=torch.Generator(device=device))
